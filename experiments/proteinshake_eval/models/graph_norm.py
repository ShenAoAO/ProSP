...
class GINConv(gnn.MessagePassing):
    ...
    def forward(self, x, edge_index, c,edge_attr=None):
        ...
        self.mlp[1].weight = self.weight_mlp(c)
        self.mlp[1].bias = self.bias_mlp(c)
...
class GNN(nn.Module):
    def __init__(self, embed_dim=256, num_layers=3, dropout=0.0, gnn_type='gin',
        ...
        for _ in range(num_layers):
            self.liners.append(nn.Linear(256+64,256))
    ...
    def forward(self, data):
        ...
        for layer in range(self.num_layers):
            output = self.gnns[layer](output, edge_index, edge_attr,prompt_atom)


            # output = self.liners[layer](torch.cat([output,prompt_atom],dim=-1))
