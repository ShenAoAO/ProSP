...
def get_data_loaders(dataset, task, masks, batch_size, num_workers):
    if 'pair' in task.task_type[0]:
        ...
        # train_scale_list_2 = []
        # for a in train_dset:
        #     train_scale_list_1.append(len(a[0].x))
        #     train_scale_list_2.append(len(a[1].x))
        # torch.save(train_scale_list_1, 'train_scale_list_ss_seq_1.npz')
        # torch.save(train_scale_list_2, 'train_scale_list_ss_seq_2.npz')
    ...
    test_loader = DataLoader(
        ...
        batch_size=batch_size, shuffle=False, num_workers=num_workers)
    # train_scale_list = []
    # for a in Subset(dataset, np.asarray(task.train_index)[train_mask]):
    #     train_scale_list.append(len(a.x))
    # torch.save(train_scale_list,'train_scale_list_bs_seq.npz')
    return train_loader, val_loader, test_loader
